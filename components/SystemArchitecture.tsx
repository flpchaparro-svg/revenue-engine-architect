...
const PILLAR_DATA = [
  ...
  { 
    ...
    modalDesc: "The single source of truth. We track every call, email, and deal so you never lose revenue to human forgetfulness.",
  ...
  { 
    ...
    modalDesc: "Content as a supply chain. We turn one hour of your raw expertise into a month of social media authority.",
      ...
      >
        ...
        <div className="p-8 md:p-10">
            <div className="flex items-center gap-3 mb-4">
               <span className="font-mono text-[10px] tracking-[0.2em] uppercase font-bold" style={{ color: data.color }}>
                  {data.group}
               </span>
            ...
            <p className="font-sans text-base text-[#1a1a1a]/70 leading-relaxed mb-8 mt-4">{data.modalDesc}</p>
            ...
            <div className="space-y-4 pt-6 border-t border-gray-100">
                {data.modalFeatures.map((feature, i) => (
                    <div key={i} className="flex items-center gap-3 group">
                        ...
                        <span className="font-mono text-[11px] text-[#1a1a1a]/70 uppercase tracking-[0.2em]">{feature}</span>
                    </div>
                ))}
            </div>
            
            <div className="mt-8 pt-6">
                <button className="flex items-center gap-2 text-[10px] font-mono uppercase tracking-[0.2em] font-bold hover:gap-4 transition-all" style={{ color: data.color }}>
      ...
      <div className="sticky top-0 h-screen w-full overflow-hidden flex items-center justify-center">
        ...
        <svg className="absolute inset-0 w-full h-full pointer-events-none z-0 hidden md:block">
          {PILLAR_DATA.map((card, i) => (
             <motion.line 
                key={i} 
                x1="50%" y1="50%" x2={card.x} y2={card.y} 
                stroke={card.color} strokeWidth="1" strokeDasharray="4 4" 
                style={{ pathLength: lineDraw, opacity: 0.3 }} 
             />
          ))}
        </svg>
        ...
        <div className="relative w-full max-w-7xl h-full mx-auto pointer-events-none flex flex-col items-center justify-center">
            ...
            <motion.div style={{ top: acqY, scale: engineScale, x: "-50%" }} className="absolute left-1/2 z-30">
               <div className="relative flex items-center justify-center md:justify-start">
                 ...
                 <motion.div 
                    ...
                    className="absolute w-64 text-center left-1/2 -translate-x-1/2 bottom-32 md:left-full md:bottom-auto md:translate-x-0 md:text-left md:ml-8 md:w-80"
                 >
                    <span className="font-mono text-[9px] text-[#E21E3F] tracking-[0.2em] uppercase font-bold block mb-2 md:mb-1">THE FOUNDATION</span>
                    <h3 className="font-serif text-3xl md:text-4xl text-[#1a1a1a] mb-2 tracking-tight">ACQUISITION</h3>
            ...
            <motion.div style={{ top: velY, scale: engineScale, x: "-50%" }} className="absolute left-1/2 z-20">
               <div className="relative flex items-center justify-center md:justify-end">
                  ...
                  <motion.div 
                    style={{ opacity: textOpacity }} 
                    className="absolute w-64 text-center left-1/2 -translate-x-1/2 bottom-32 md:right-full md:bottom-auto md:translate-x-0 md:text-right md:mr-8 md:w-80"
                  >
                    <span className="font-mono text-[9px] text-[#C5A059] tracking-[0.2em] uppercase font-bold block mb-2 md:mb-1">THE AMPLIFIER</span>
                    <h3 className="font-serif text-3xl md:text-4xl text-[#1a1a1a] mb-2 tracking-tight">VELOCITY</h3>
            ...
            <motion.div style={{ top: intY, scale: engineScale, x: "-50%" }} className="absolute left-1/2 z-10">
               <div className="relative flex items-center justify-center md:justify-start">
                  ...
                  <motion.div 
                    ...
                    className="absolute w-64 text-center left-1/2 -translate-x-1/2 bottom-32 md:left-full md:bottom-auto md:translate-x-0 md:text-left md:ml-8 md:w-80"
                  >
                    <span className="font-mono text-[9px] text-[#1a1a1a] tracking-[0.2em] uppercase font-bold block mb-2 md:mb-1">THE NAVIGATOR</span>
                    <h3 className="font-serif text-3xl md:text-4xl text-[#1a1a1a] mb-2 tracking-tight">INTELLIGENCE</h3>
        ...
        {/* FLOATING LABELS (Hidden on Mobile, Visible on Desktop) */}
        <motion.div style={{ opacity: cardsOpacity }} className="absolute inset-0 pointer-events-none z-40 hidden md:block">
           {PILLAR_DATA.map((card) => (
              <motion.div
                ...
                className="absolute cursor-pointer bg-[#FFF2EC] border rounded-sm p-5 w-60 flex flex-col items-center text-center shadow-lg pointer-events-auto transition-colors duration-300 hover:bg-white group"
                onClick={() => setSelectedPillar(card)}
              >
                 <span className="text-[9px] font-mono uppercase tracking-[0.2em] font-bold mb-1.5 opacity-80" style={{ color: card.color }}>{card.group}</span>
                 <h4 className="font-serif text-xl text-[#1a1a1a] mb-1 leading-none">{card.title}</h4>
                 <p className="text-[9px] font-mono text-[#1a1a1a]/60 tracking-[0.1em] mb-2 block uppercase">{card.subtitle}</p>
        ...
        <AnimatePresence>
            {selectedPillar && (
                <SystemModal data={selectedPillar} onClose={() => setSelectedPillar(null)} />
            )}
        </AnimatePresence>
      </div>
    </div>
